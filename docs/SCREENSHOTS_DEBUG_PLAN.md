...
| 2026-09-04 20:11Z | ios-screenshots-debug (run 33914945427) | `f69d773` | continue login when an FCM token is unavailable | 25.9 s | mechanical pass; visual QA failed because 03-Home contains a notification prompt | **H2 fixed**; duplicate Home permission request live | Captured 01-04 and 06-10, all 1320x2868; optional 05-CharterDetails was skipped because no detail card was available. 03-Home and charter lists had no assigned charter data. App PID 39122 remained constant; the only `.ips` was for unrelated `UsageTrackingAgent`. |
| 2026-09-04 20:41Z | ios-screenshots-debug (run 33917508935) | `8ef23d2` | remove duplicate Home notification permission request | 31.7 s | test passed; Phase 3 blocked by empty charter data | duplicate permission prompt fixed | Captured 01-04 and 06-10 (9 PNGs), all 1320x2868, with no alerts or spinners. Home, Today's Charter, Upcoming Charter, and Charter History were empty, so the demo account needs an assigned charter before final validation. App PID 37896 remained constant; the only `.ips` was for unrelated `UsageTrackingAgent`. |
| 2026-09-05 00:06Z | ios-screenshots-debug (run 33931862383) | `b5f85d9` | Phase 3 validation 1 after assigning demo charter data | 31.6 s | test passed; Phase 3 content validation failed | charter assignment not visible to the app | Captured 01-04 and 06-10 (9 PNGs), all 1320x2868, with no alerts or spinners. Home and Today's Charter still showed `NO CHARTER FOUND`; 05-CharterDetails was not captured. App PID 33227 remained constant; the only `.ips` was for unrelated `UsageTrackingAgent`. |
| 2026-09-05 00:31Z | ios-appstore-release (run 33933277343) | `6b2e80a` | true 1.0.3 App Store release with review submission | n/a | XCTest timed out requesting app launch progress before automation connected | H5 runner/simulator starvation | Signed IPA build passed in 4m17s, but simulator boot took 125 s and app launch timed out after 185 s. No `fti_coach` `.ips`; publish/submission/tagging did not run, so retry is safe. |
| 2026-09-05 01:16Z | ios-appstore-release (run 33935686113) | `a59f828` | retry true 1.0.3 App Store release with review submission | 18.7 s | none; release passed | release pipeline validated | Built and uploaded the signed IPA, captured and uploaded 9 PNGs at 1320x2868, submitted 1.0.3 for App Store review, then pushed release commit `0bd3a12` and tag `v1.0.3`. App PID 46674 remained constant. |
| 2026-09-22 15:02Z | ios-screenshots-debug (run 35744513601) | `56aa0d8` | password-validation fix + ChangePassword state captures via SCREENSHOT_DEBUG_STATES | 11.0 s | none; test passed; typed states not rendered | n/a (harness issue) | 09b/09c PNGs byte-identical to 09-ChangePassword despite successful typeText. App PID constant. |
| 2026-09-22 15:02Z | ios-screenshots-debug (run 35744517791) | `56aa0d8` | SCREENSHOT_START_SCREEN=NewPasswordScreen hook | n/a | none; test passed; typed states not rendered | start-screen hook works | App opened directly on the OTP-gated screen; 21/22 PNGs byte-identical to 20. |
| 2026-09-22 15:42Z | ios-screenshots-debug (run 35749197780) | `61581d7` | field-value diagnostics after typeText | 11.8 s | none; test passed | rendering-only issue, input lands | AX value = 9 bullets for both fields; PNGs still byte-identical to load; no retry needed. |
| 2026-09-22 15:42Z | ios-screenshots-debug (run 35749200317) | `61581d7` | same diagnostics on NewPasswordScreen | n/a | none; test passed | rendering-only issue, input lands | AX value = 9 bullets; PNGs byte-identical. Secure bullets do not paint in the simulator build (02-SignIn password is also empty although login succeeds). |
| 2026-09-22 16:18Z | ios-screenshots-debug (runs 35753223811, 35753227799) | `514a87c` | coordinate eye-toggle reveal attempt | n/a | none; tests passed | library toggle unresponsive to coordinate taps | Pixel diff showed only the status-bar clock changed; icon never flipped. |
| 2026-09-22 16:52Z | ios-screenshots-debug (runs 35756986391, 35756989314) | `5680384` | `<field>-eye` testID on the toggle + element tap | n/a | none; tests passed | toggle still unresponsive | `-eye` found (exists=true) and tap succeeded; icon never flipped; text never painted. |
| 2026-09-22 17:21Z | ios-screenshots-debug (run 35760131879) | `df4b79b` | frame + AX dump diagnostics | n/a | none; test passed | n/a | eye frame sane (right edge of row); AX dump truncated in log; loop closed. |
| 2026-09-22 17:49Z | ios-screenshots-debug (runs 35763092528, 35763095277) | `eddc7fb` | final: value-logged typed proof | n/a | none; both passed | validation complete | AX value = expected masked bullets on all four typed fields; 13 + 3 PNGs captured; no submits. |
